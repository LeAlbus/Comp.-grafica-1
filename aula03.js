...
function main() {
	...
	//Modelos
	model2 = mat4.create();
	model2 = mat4.translate([], model2, [0,-1.1,0]);
	gl.uniformMatrix4fv(modelLocation, false, new Float32Array(model2));
	//gl.clear( gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT );
	gl.drawArrays(gl.TRIANGLES, 0, data.points.length/3);
...
function direita(){
	model = mat4.rotate([], model, 5*Math.PI/180, [0, 0, 1]);
...
function draw(){
	...
	model = mat4.rotate([], model,5*Math.PI/180,[0,1,0]);
	//model = mat4.scale([], model, [1.1,1.1,1.1]);
	//model = mat4.translate([], model, [0,0.1,0]);
